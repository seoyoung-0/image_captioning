    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        if not self.use_bert:
            ...
            for cap_idx in encoded_captions:
                ...
                full_sentence = ''
                for i, tok in enumerate(tokenized_cap):
                    if tok=='.' or tok=='[SEP]' or tok=='[PAD]' or tok=='[UNK]':
                        full_sentence += ' ' + tok
                    elif tok=='▁':
                        full_sentence += ' ▁ '
                    else:
                        full_sentence += tok.replace('▁', ' ')
                split_sen = full_sentence.split()
                ...
                for full_token in split_sen:
                    curr_token = ''
                    x = 0
                    for i,_ in enumerate(tokenized_cap[1:]): # disregard CLS
                    
                        token = tokenized_cap[i+j].replace('▁', '')
                        ...
                        if full_token=='▁':
                            ...
                            j += 1
                            break
                        elif token == full_token and curr_token == '' : # full token
                            tokens_embedding.append(piece_embedding)
                            j += 1
                            break
                        else: # partial token
